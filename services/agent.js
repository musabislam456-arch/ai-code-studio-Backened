...
import {
  initRepo, commitAll, push, pull, unlinkRemote, getRemoteInfo, createGithubRepoAndPush
} from "./git.js";
...
// Git/GitHub tools are now fully implemented in makeToolExecutor below (via
// services/git.js), so they're declared to the model like every other tool.
// (Previously this filter excluded them entirely, which meant Gemini could
// never even attempt a commit/push/pull — that, combined with the missing
// executor cases, was the root cause of "no model can push/pull".)
const TOOLS = [
...
export function makeToolExecutor(workspaceDir, onStep) {
  return async function execTool(name, args = {}) {
    switch (name) {
      ...
      // Git/GitHub tools — same services/git.js functions the manual
      // GitPanel REST routes (routes/api.js) call, just exposed to the
      // agent's tool-calling loop too. initRepo() is safe to call
      // repeatedly (no-op if already a repo).
      case "github_status": {
        await initRepo(workspaceDir);
        return await getRemoteInfo(workspaceDir, "origin");
      }
      case "github_create_repo": {
        const token = process.env.GITHUB_TOKEN;
        if (!token) return { error: "GITHUB_TOKEN server par set nahi hai — Railway env vars mein add karo." };
        if (!args.name?.trim()) return { error: "Repository name required hai." };
        ...
        return { ok: true, htmlUrl: result.htmlUrl, cloneUrl: result.cloneUrl, pushed: result.pushed, branch: result.branch };
      }
      case "github_unlink": return await unlinkRemote(workspaceDir, "origin");
      case "git_commit": {
        await initRepo(workspaceDir);
        return await commitAll(workspaceDir, args.message);
      }
      case "git_push": {
        await initRepo(workspaceDir);
        return await push(workspaceDir, "origin", "main");
      }
      case "git_pull": {
        await initRepo(workspaceDir);
        return await pull(workspaceDir, "origin", "main");
      }
